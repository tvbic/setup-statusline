...
腳本來源位於此 command 的上層 plugin 目錄內的 `scripts/` 資料夾。

先用 Bash 工具找到腳本的實際路徑：
```bash
find ~/.claude -path "*/setup-statusline/scripts/statusline.ps1" -o -path "*/setup-statusline/scripts/statusline.sh" 2>/dev/null
```

找到後，將對應腳本複製到 `~/.claude/` 目錄下：

**Windows:**
```
複製 <找到的 statusline.ps1 路徑> → ~/.claude/statusline.ps1
```

**macOS/Linux:**
```
複製 <找到的 statusline.sh 路徑> → ~/.claude/statusline.sh
